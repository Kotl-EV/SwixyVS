...
namespace SwixyQuestBook.Gui
{
    ...
    internal sealed class QuestbookGuiItemIconRenderer
    {
        ...
        /// <summary>Must be the inventory's own slot — some blocks call MarkDirty during GUI render.</summary>
        private readonly ItemSlot renderSlot;

        public QuestbookGuiItemIconRenderer(ICoreClientAPI api)
        {
            this.api = api;
            // quantitySlots:1 so GenEmptySlots creates slots[0] that MarkDirty can resolve.
            dummyInventory = new DummyInventory(api, 1);
            dummyInventory.OnAcquireTransitionSpeed += static (_, _, _) => 0;
            renderSlot = dummyInventory[0];
        }
            ...
            double clipHeight = double.NaN)
        {
            ...
            try
            {
                // Some collectibles (cooked pots, transition items) throw during GUI render —
                // never let one bad catalog tile crash the whole client.
