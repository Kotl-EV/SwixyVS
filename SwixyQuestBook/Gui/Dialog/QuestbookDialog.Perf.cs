...
namespace SwixyQuestBook.Gui
{
    ...
    public sealed partial class QuestbookDialog
    {
        ...
        private int graphCacheCraftHash;
        ...
        private void EnsureGraphCache(QuestbookCategoryDefinition category)
        {
            ...
            RebuildGraphCache(category, invHash, craftHash, admin);
        }

        /// <summary>
        /// Fingerprint of server craft progress so graph ready/percent refresh when
        /// crafting updates without an inventory change (and ignore creative-only items).
        /// </summary>
        private int BuildCraftProgressFingerprint()
        {
            int hash = 17;
            foreach ((string key, int count) in dataManager.CraftProgressMap)
            {
                hash = (hash * 31) ^ StringComparer.OrdinalIgnoreCase.GetHashCode(key);
                hash = (hash * 31) ^ count;
            }

            // Kill progress also drives ready/percent without inventory changes.
            foreach ((string key, int count) in dataManager.KillProgressMap)
            {
                hash = (hash * 31) ^ StringComparer.OrdinalIgnoreCase.GetHashCode(key);
                hash = (hash * 31) ^ count;
            }
        ...
        private void RebuildGraphCache(QuestbookCategoryDefinition category, int invHash, int craftHash, bool admin)
        {
            graphNodeById.Clear();
            ...
            foreach (QuestbookQuestNodeDefinition node in category.Nodes)
            {
                ...
                bool ready = false;
                if (!admin && !completed && unlocked && node.IsObjectiveNode)
                {
        ...
        {
            // Keep in sync with IsNodeReadyToSubmit (craft vs inventory goals).
