...
/// </summary>
public sealed class IslandMemberListCell : GuiElementTextBase, IGuiElementCell
{
    ...
    /// Колонка Delete рисуется отдельно через <see cref="IslandCairoIcons.DrawTrash"/>.
    /// </summary>
    /// <param name="ctx">Контекст Cairo.</param>
    ...
    private void DrawColumnIcon(Context ctx, double x, double y, double size, HoverRegion column)
    {
        switch (column)
        {
            case HoverRegion.Owner:
                IslandCairoIcons.DrawOwner(ctx, x, y, size, CrownState);
                break;
            case HoverRegion.Use:
                IslandCairoIcons.DrawGear(ctx, x, y, size, AccessUse, IsOwner);
                break;
            case HoverRegion.Build:
                IslandCairoIcons.DrawPickaxe(ctx, x, y, size, AccessBuild, IsOwner);
                break;
        }
    }
